...
export class GameAction7_5 {
  line: number[][] = []
  score: number = 0
}
export class module7_5 {
  getRiddle() {
    let desk = new GameData7_5();
    return desk;
  }
  checkRiddle(desk: GameData7_5) {
    if (desk.sizeX > 5 || desk.sizeX < 2) {
      return false
    }
    return true
  }
  checkDesk(desk: GameData7_5) {
  ...
  doAction(deskIn: GameData7_5, act: GameAction7_5): [flag: number, desk: GameData7_5] {
    let desk: GameData7_5 = _.cloneDeep(deskIn)
  ...
  checkAction(desk: GameData7_5, act: GameAction7_5) {
    for (let i = 0; i < act.line.length; i++) {
      let [x, y] = act.line[i];
      if (x > desk.sizeX || y > desk.sizeY || x < 0 || y < 0) {
        // 超出棋盘范围
        // console.log('超出棋盘范围')
        return false
      }

      let isTopRight = y == 0 && x == desk.sizeX;
      if (isTopRight) {
        return false
      }
  ...
  getActionAuto(desk: GameData7_5): any[] {
  ...
  getActionAll(desk: GameData7_5): GameAction7_5[] {
    let listActionAll: GameAction7_5[] = []
    // 从左上到右下遍历，每个向右向下取点
    for (let y = 0; y <= desk.sizeY; y++) {
      for (let x = 0; x <= desk.sizeX; x++) {
        let pEnd1 = [x + 1, y]
        let act1 = new GameAction7_5();
        act1.line = [[x, y], pEnd1]
        if (this.checkAction(desk, act1)) {
          listActionAll.push(act1)
        }
        let pEnd2 = [x, y + 1]
        let act2 = new GameAction7_5();
        act2.line = [[x, y], pEnd2]
