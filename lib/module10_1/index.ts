...
export default class example10_1 {
  ...
  doAction(
    deskData: GameData10_1,
    dataAction: GameAction10_1
  ): [flagResult: number, dataResult: GameData10_1] {
  ...
  getActionAuto(deskData: GameData10_1): GameAutoWay {
    ...
    let cando =
      deskData.desk[x][y] == deskData.player &&
      deskData.desk[y][x] == deskData.player;
    while (cando) {
    ...
    // }
    if (bs > nbs) {
      return new GameAutoWay(
        new GameAction10_1(x, y),
        new GameAction10_1(x, y)
      );
    } else if (
      bs == nbs &&
      deskData.desk[x][y] == OtherUtil.getRival(deskData.player)
    ) {
      return new GameAutoWay(
        new GameAction10_1(x, y),
        new GameAction10_1(x, y)
      ...
      bs == nbs &&
      deskData.desk[y][x] == OtherUtil.getRival(deskData.player)
    ) {
      return new GameAutoWay(
        new GameAction10_1(x, y),
        new GameAction10_1(x, y)
  ...
  lurd(deskData: GameData10_1, x: number, y: number): boolean {
    ...
    if (cell != 0) {
      for (let i = -2; i <= 2; i++) {
        ...
        // if (this.vaildXy(x1, y) && this.vaildXy(x2, y) && this.vaildXy(x3, y)) {
        if (
          this.vaildXy(tx1, ty1) &&
          this.vaildXy(tx2, ty2) &&
          this.vaildXy(tx3, ty3)
        ) {
